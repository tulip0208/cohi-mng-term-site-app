...
import { View, Text, TouchableOpacity, Modal, ScrollView, TextInput,KeyboardAvoidingView } from 'react-native';
...
import { WA1060PrevScreenId,WA1061BackState,WA1060NewTagIdState,WA1060OldTagInfosState,WA1060WkPlacState,WA1061TagIdState } from "../atom/atom.tsx";
import { CT0007,CT0006,CT0005,CT0008,CT0009,CT0010,CT0011} from "../enum/enums.tsx";
...
const WA1061 = ({navigation}:Props) => {
    ...
    const [WA1061TagId,setWA1061TagId] = useRecoilState(WA1061TagIdState);
    ...
    const handleCodeScannedForTag = async (data:string,type:string) => {
      ...
      if (type !== RNCamera.Constants.BarCodeType.qr && type !== 'CODABAR') {
        ...
        setWA1060OldTagInfos([...WA1060OldTagInfos,{
          ...
          ocloc:parts[5],//発生場所
          rmSolInf:(isOrange) ?  "調査結果：土壌環境及び土壌溶出基準出不適合" : parts[18],//備考(除去土壌情報) 
    ...
    const newOldJudge = async (rmSolTyp:string) :Promise<boolean>=> {
      ...
      switch(newTag.charAt(0)){
        ...
        case '5'://赤色
          ...
          break;
        case '7'://橙色
          judge=['1','2','3','4','5','6','10','11','12','13'].includes(rmSolTyp)
          break;
      }
      ...
      if (WA1060OldTagInfos.length < 9) {
        const configComponent = (
          <View key={WA1060OldTagInfos.length+1} style={[styles.detailSection]}>
            ...
            <View style={[styles.tableCell1]}>
              <TouchableOpacity style={[styles.detailButton, styles.updateButton]}
                onPress={async () => {
      ...
      >
        <ScrollView  contentContainerStyle={[styles.containerWithKeybord, { flexGrow: 1 }]}>
          ...
          <View style={styles.bottomSection}>
            <TouchableOpacity style={[styles.buttonNarrow,styles.centerButtonNarrow]} onPress={btnTagQr}>
              <Text style={styles.buttonText}>タグ読込</Text>
            </TouchableOpacity>
            <TouchableOpacity style={[styles.buttonNarrow,styles.centerButtonNarrow,styles.dummyTagButton]} onPress={btnDummyTag}>
