...
const WA1131 = ({navigation}:Props) => {
    ...
    const btnAppSend = async () => {
      ...
      if(WA1130Data.facTyp === "0"){//0:定置場の場合
        tripStatusKbn = '10'//輸送ステータス区分　10:荷下完了
      }else if(WA1130Data.facTyp === "1"){//1:受入・分別施設
      ...
      //通信処理 IFT0120
      const responseIFA0120 = await IFT0120(
        WA1130Data,
        IFT0640Data,
        dateStr,
      );
      ...
      if(responseIFA0120.data?.itcRstCd===1){
        ...
        const errorDetails: [string, string][] = responseIFA0120.data?.invDatDtl.map(dtl => {
        ...
        await showAlert("通知", messages.EA5030("荷下定置ステータス更新", ...errorDetails), false);
        return;
      }
    ...
    const procNewTagId = async (tag:string) => {
      ...
      const newLgSdBgDtl = IFT0640Data.lgSdBgDtl.map(item => {
        if (item.newTagId === tag) {
          ...
          if (!item.check) allChecked = false;
    ...
    return (
      <View style={styles.container}>
        {/* ヘッダ */}
        <FunctionHeader appType={"現"} viewTitle={"新タグ読込"} functionTitle={"荷下登録"}/>
    
        {/* 上段 */}
        <View  style={[styles.main]}>
          <Text style={[styles.labelText]}>作業場所：{WA1130Data.wkplcTyp}</Text>
          <Text style={[styles.labelTextNarrow,styles.labelTextPlace]}>{WA1130Data.wkplc}</Text>
        </View>
        ...
        <View  style={[styles.textareaContainer,styles.topContent]}>
          <View style={styles.tableMain}>
            <View style={styles.tableRow}>
              ...
              <View style={styles.tableCell}><Text style={styles.labelTextNarrow}>{IFT0640Data.crdNo}</Text></View>
            </View>
            <View style={styles.tableRow}>
              <View style={styles.tableCell}><Text style={[styles.labelTextNarrow,styles.alignRight]}>輸送車両：</Text></View>
              <View style={styles.tableCell}><Text style={styles.labelTextNarrow}>{IFT0640Data.vclNum}</Text></View>
            </View>
            <View style={styles.tableRow}>
              <View style={styles.tableCell}><Text style={[styles.labelTextNarrow,styles.alignRight]}>運転手：</Text></View>
              <View style={styles.tableCell}><Text style={styles.labelTextNarrow}>{IFT0640Data.drvName}</Text></View>
            </View>
          </View>  
