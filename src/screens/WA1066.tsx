...
import { WA1061BackState,WA1060PrevScreenId,WA1060DataState,WA1060NewTagIdState,WA1060CmnTagFlgState,WA1060OldTagInfosState,WA1063MemoAutoState,WA1060WkPlacState,WA1065MemoState } from "../atom/atom.tsx";
...
import { CT0007,CT0006,CT0005,CT0008,CT0009,CT0010,CT0011,CT0042} from "../enum/enums.tsx";
...
const WA1066 = ({navigation}:Props) => {
    ...
    const [selectedOldTagInfo, setSelectedOldTagInfo] = useState<WA1060OldTagInfoConst | null>(null);
      ...
      return(
        <View style={styles.tableMain}>
          <View style={styles.tableRow}>
            <View style={styles.tableCell}><Text style={styles.labelText}>津波浸水：{CT0005[Number(oldTagInfo?.tsuInd)] ?? ''}</Text></View>
          </View>
          <View style={styles.tableRow}>
            <View style={styles.tableCell}><Text style={styles.labelText}>特定施設：{CT0006[Number(oldTagInfo?.splFac)] ?? ''}</Text></View>
          </View>
          <View style={styles.tableRow}>
            <View style={styles.tableCell}><Text style={styles.labelText}>除去土壌等種別：{CT0007[Number(oldTagInfo?.rmSolTyp)] ?? ''}</Text></View>
          </View>
          <View style={styles.tableRow}>
            <View style={styles.tableCell}><Text style={styles.labelText}>発生土地分類：{CT0008[Number(oldTagInfo?.ocLndCla)] ?? ''}</Text></View>
          </View>
          <View style={styles.tableRow}>
            <View style={styles.tableCell}><Text style={styles.labelText}>荷姿種別：{CT0009[Number(oldTagInfo?.pkTyp)] ?? ''}</Text></View>
          </View>
          <View style={styles.tableRow}>
            <View style={styles.tableCell}><Text style={styles.labelText}>内袋の利用方法：{CT0010[Number(oldTagInfo?.usgInnBg)] ?? ''}</Text></View>
          </View>
          <View style={styles.tableRow}>
            <View style={styles.tableCell}><Text style={styles.labelText}>アルミ内袋の利用：{CT0011[Number(oldTagInfo?.usgAluBg)] ?? ''}</Text></View>
          </View>
          <View style={styles.tableRow}>
            <View style={styles.tableCell}><Text style={styles.labelText}>容積：{oldTagInfo?.vol}</Text></View>
          </View>
          <View style={styles.tableRow}>
            <View style={styles.tableCell}><Text style={styles.labelText}>空間線量率：{oldTagInfo?.airDsRt}</Text></View>
          </View>
          <View style={styles.tableRow}>
            <View style={styles.tableCell}><Text style={styles.labelText}>発生土地の利用区分：{oldTagInfo?.ocLndUseknd}</Text></View>
          </View>
          <View style={styles.tableRow}>
            <View style={styles.tableCell}><Text style={styles.labelText}>発生場所：{oldTagInfo?.ocloc}</Text></View>
          </View>
          <View style={styles.tableRow}>
            <View style={styles.tableCell}><Text style={styles.labelText}>備考(除去土壌情報)：</Text></View>
          </View>
          <View style={styles.tableRow}>
            <View style={styles.tableCell}><Text style={styles.labelText}>{oldTagInfo?.rmSolInf}</Text></View>
          </View>
          <View style={styles.tableRow}>
            <View style={styles.tableCell}><Text style={styles.labelText}>除染時データメモ：</Text></View>
          </View>
          <View style={styles.tableRow}>
            <View style={styles.tableCell}><Text style={styles.labelText}>{oldTagInfo?.lnkNewTagDatMem}</Text></View>
          </View>
        </View>
      );
    };
    ...
    return (
      ...
      >
        <ScrollView  contentContainerStyle={[styles.containerWithKeybord, { flexGrow: 1 }]}>
          ...
          {/* ヘッダ */}
          <FunctionHeader appType={"現"} viewTitle={"登録内容確認"} functionTitle={"紐付(土)"}/>
          {/* 上段 */}
          <View  style={[styles.main]}>
                    ...
                    <View style={[styles.tableCell1]}>
                      <TouchableOpacity 
                        ...
                        onPress={async () => {
                          ...
                          await logUserAction(`ボタン押下: 詳細(`+oldTagInfo.oldTag+`)(WA1061)`);
                        }}
                      >
            ...
            <View style={styles.tableMain}>
              <View style={[styles.tableRow,styles.pickerContainer]}>
                ...
                <View style={[styles.tableCell]}><Text style={styles.pickerLabelText}>{CT0007[Number(WA1060Data.rmSolTyp)] ?? ''}</Text></View>
              </View>
              <View style={[styles.tableRow,styles.pickerContainer]}>
                <View style={styles.tableCell}><Text style={[styles.pickerLabelText,styles.alignRight]}>荷姿種別：</Text></View>
                <View style={[styles.tableCell]}><Text style={styles.pickerLabelText}>{CT0009[Number(WA1060Data.pkTyp)] ?? ''}</Text></View>
              </View>
              <View style={[styles.tableRow,styles.pickerContainer]}>
                <View style={styles.tableCell}><Text style={[styles.pickerLabelText,styles.alignRight]}>内袋の利用方法：</Text></View>
                <View style={[styles.tableCell]}><Text style={styles.pickerLabelText}>{CT0010[Number(WA1060Data.usgInnBg)] ?? ''}</Text></View>
              </View>
              <View style={[styles.tableRow,styles.pickerContainer]}>
                <View style={styles.tableCell}><Text style={[styles.pickerLabelText,styles.alignRight]}>津波浸水域由来：</Text></View>
                <View style={[styles.tableCell]}><Text style={styles.pickerLabelText}>{CT0005[Number(WA1060Data.tsuInd)] ?? ''}</Text></View>
              </View>
              <View style={[styles.tableRow,styles.pickerContainer]}>
                <View style={styles.tableCell}><Text style={[styles.pickerLabelText,styles.alignRight]}>特定施設由来：</Text></View>
                <View style={[styles.tableCell]}><Text style={styles.pickerLabelText}>{CT0006[Number(WA1060Data.splFac)] ?? ''}</Text></View>
              </View>
              <View style={[styles.tableRow,styles.pickerContainer]}>
                <View style={styles.tableCell}><Text style={[styles.pickerLabelText,styles.alignRight]}>アルミ内袋の利用：</Text></View>
                <View style={[styles.tableCell]}><Text style={styles.pickerLabelText}>{CT0011[Number(WA1060Data.usgAluBg)] ?? ''}</Text></View>
              </View>            
              <View style={[styles.tableRow,styles.pickerContainer]}>
                <View style={styles.tableCell}><Text style={[styles.pickerLabelText,styles.alignRight]}>オーバーパック有無：{CT0042[Number(WA1060Data.yesNoOP)] ?? ''}</Text></View>
              </View>            
            </View>
            ...
            <View style={styles.tableMain}>
              <View style={[styles.tableRow,styles.pickerContainer]}>
                ...
                <View style={[styles.tableCell]}><Text style={styles.pickerLabelText}>{WA1060Data.caLgSdBgWt ?? ''}</Text></View>
              </View>
              <View style={[styles.tableRow,styles.pickerContainer]}>
                <View style={styles.tableCell}><Text style={[styles.pickerLabelText,styles.alignRight]}>線量(μSv/h)：</Text></View>
                <View style={[styles.tableCell]}><Text style={styles.pickerLabelText}>{WA1060Data.caLgSdBgDs ?? ''}</Text></View>
              </View>
              <View style={[styles.tableRow,styles.pickerContainer]}>
                <View style={styles.tableCell}><Text style={[styles.pickerLabelText,styles.alignRight]}>推定放射能濃度：</Text></View>
                <View style={[styles.tableCell]}><Text style={styles.pickerLabelText}>{WA1060Data.estRa}</Text></View>
              </View>
              <View style={styles.tableRow}>
            ...
            <View style={styles.memoScrollContainer}>
              <View style={styles.memoScroll}>
                <ScrollView nestedScrollEnabled={true}>
                  <Text style={[styles.memo,styles.textBlack]}>{WA1063MemoAuto+WA1065Memo}</Text>
                </ScrollView>
              </View>
            </View>
