...
const WA1020 = ({navigation}:Props) => {
    ...
    const [showScannerActivate, setShowScannerActivate] = useState<boolean>(false); // カメラ表示用の状態
    ...
    return (
      <View style={styles.container}>
        ...
        <View  style={[styles.main,styles.topContent]}>
          ...
          <TouchableOpacity style={[styles.button,styles.buttonRead]} onPress={btnUserQr}>
            <Text style={styles.buttonText}>利用者読込</Text>
          </TouchableOpacity>
          <Text style={styles.labelText}>アクティベーションコード読込：{actReadFlg}</Text>
          <TouchableOpacity style={[styles.button,styles.buttonRead]} onPress={btnActQr}>
