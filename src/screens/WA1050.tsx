  ...
  const WA1050 = ({route,navigation}:Props) => {
      ...
      const btnDelLog = async () => {
        ...
        if (IA5010_choise) {
          ...
          await deleteLogs(async (title, message, showCancel) => {await showAlert(title, message, showCancel)});
          const logSize = await calculateTotalLogSize();
          setLogCap(logSize);
        }
      };
      ...
      const btnUpLog = async () => {
        ...
        if (IA5006_choise) {
          ...
          // 位置情報取得を停止
          if(global.locationStarted) await clearLocation();
          ...
          // 位置情報取得を停止した場合再開する
          if(global.locationStopped) await watchLocation();
          // モーダル非表示
