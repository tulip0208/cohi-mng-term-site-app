...
import { WA1061BackState,WA1060NewTagIdState,WA1060OldTagInfosState,WA1061TagIdState,WA1063MemoAutoState,WA1060PrevScreenId } from "../atom/atom.tsx";
...
const WA1062 = ({navigation}:Props) => {
    ...
    const [selectLnkNewTagDatMem,setSelectLnkNewTagDatMem] = useState<string>("");
    const [isLnkNewTagDatMemInpDisabled,setIsLnkNewTagDatMemInpDisabled] = useState<boolean>(true);
    ...
    const btnSetting = async () => {
      ...
      setWA1060OldTagInfos([...WA1060OldTagInfos,{
        ...
        ocloc:"",//発生場所
        rmSolInf:(selectLnkNewTagDatMem === 'その他') ? lnkNewTagDatMemInp : selectLnkNewTagDatMem,//備考(除去土壌情報) 
        lnkNewTagDatMem:"",//除染時データメモ
      }]);

      //除去土壌等種別が変更された場合(既に旧タグがある場合 かつ ダミータグではない場合)
      if(selectRmSolTypeInit != selectRmSolType && "dummyTag" !== WA1061TagId[1]){
        if(WA1063MemoAuto){
          setWA1063MemoAuto(WA1063MemoAuto + ',' + WA1061TagId + ':除去土壌等種別が異なる')
        }else{
    ...
    const getTextInputStyle = () =>{
      return isLnkNewTagDatMemInpDisabled ?  [styles.textAreaInput,styles.inputDisabled] : styles.textAreaInput ;
    }
    ...
    const filterText = async () => {
      try {
        ...
        let filteredText: string = await JISInputFilter.checkJISText(lnkNewTagDatMemInp);
        if(!filteredText){
          filteredText = '';
        }
    ...
    return (
      ...
      >
        <ScrollView  contentContainerStyle={[styles.containerWithKeybord, { flexGrow: 1 }]}>
          {/* ヘッダ */}
          <FunctionHeader appType={"現"} viewTitle={"旧タグ設定"} functionTitle={"紐付(土)"}/>
