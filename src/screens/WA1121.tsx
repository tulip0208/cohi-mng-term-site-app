...
import { View, Text, TouchableOpacity, Modal, ScrollView, TextInput,TouchableWithoutFeedback,KeyboardAvoidingView } from 'react-native';
...
import { WA1120PrevScreenId, WA1120DataState,WA1120BackState, WA1120WkPlacState, WA1120CarState, WA1120DrvState, WA1120DestState, WA1120TrpCardNoState,WA1121DataState,WA1121NewTagListState } from "../atom/atom.tsx";
import { useAlert } from '../components/AlertContext.tsx';
import messages from '../utils/messages.tsx';
import { ApiResponse, IFA0330Response,IFA0330ResponseDtl,IFA0340Response,IFA0340ResponseDtl,caLgSdBgDsInfoConst } from '../types/type';
...
const WA1121 = ({navigation}:Props) => {
    ...
    const [newTagList, setNewTagList ] = useRecoilState(WA1121NewTagListState);
    ...
    useEffect(() => {
      ...
      if(Number(WA1120Car.empCarWt)<=Number(WA1120Car.carWt)){
        ...
        wt = Number(WA1120Car.maxWt) - (Number(WA1120Car.empCarWt) - Number(WA1120Car.carWt))
      }
      setWA1121Data(data => ({
        ...data,
    ...
    const getTagReadButtonStyle = () =>{
      return isTagRead ? [styles.buttonNarrower,styles.centerButtonNarrow] : [styles.buttonNarrower,styles.centerButtonNarrow,styles.disabledButton];
    };
    ...
    const btnSend = async () => {
      ...
      if(responseIFT0130.data?.invDatDtl.length as number > 0 || 
        (responseIFT0130.data?.dtl.length as number > 0 && responseIFT0130.data?.dtl[0].trpCdAplRst === 1)){
    ...
    const onPressIn = (index: number) => {
      ...
      const timer = setTimeout(async () => {
        ...
        const newArray = WA1121Data.trpCardTagInfoList.filter((_, i) => i !== index);
    ...
    const newTagIdProc = async (tagId:string) => {
      ...
      if(freTyp != WA1121Data.freJudTyp){
        await showAlert("通知", messages.EA5016('除去土壌等、焼却灰が混載となる'), false);
        return;
      }
      ...
      if(WA1121Data.gai !== '9'){
        if(WA1121Data.gai !== freJudGai){
          await showAlert("通知", messages.EA5016('有害・無害が混載となる'), false);
          return;
        }
      }
      ...
      if(WA1121Data.freJudTyp === '1'){
        ...
        const settingsInfo = realm.objects('settings')[0];
        //新タグ情報の追加
        setWA1121Data({...WA1121Data,
          trpCardTagInfoList:[...WA1121Data.trpCardTagInfoList,{
      ...
      //----------------------------
      }else if(WA1121Data.freJudTyp === '2'){//[フレコン判定種別]＝"2:焼却灰"の場合、除去土壌の判定処理
        //通信処理
    ...
    const renderNewTagList = () => {
      ...
      //新タグリスト
      const newTagComponent = WA1121Data.trpCardTagInfoList.map((trpCardTagInfo, index) => {
        // 分類に対応する色を取得
      ...
      if (WA1121Data.trpCardTagInfoList.length < 20) {
        const configComponent = (
          <View key={WA1121Data.trpCardTagInfoList.length+1} style={[styles.detailSection]}>
            <View style={[styles.tableCell2,styles.inputContainer]}>
              <Text style={styles.labelTextNarrowMore}>{`${WA1121Data.trpCardTagInfoList.length+1}: a`}</Text>
                <TextInput 
                  value={inputValue}
            ...
            <View style={[styles.tableCell1]}>
              <TouchableOpacity style={[styles.detailButtonNarrow, styles.updateButton,styles.labelTextNarrowMore]}
                onPress={async () => {
                  await logUserAction(`ボタン押下: 設定(WA1121)`);
                  await btnSetting();
                }}
              >
                <Text style={[styles.detailButtonText, styles.settingButtonText]}>設定</Text>
              </TouchableOpacity>
            </View>
          </View>
        );
    ...
    return (
      ...
      >
        <ScrollView  contentContainerStyle={[styles.containerWithKeybord, { flexGrow: 1 }]}>
          {/* ヘッダ */}
          <FunctionHeader appType={"現"} viewTitle={"旧タグ読込"} functionTitle={"紐付(土)"}/>
      
          {/* 上段 */}
          <View style={styles.tableMain}>
            <View style={styles.tableRow}>
              <View style={styles.tableCell}><Text style={[styles.labelTextNarrowMore,styles.alignRight]}>作業場所:{WA1120Data.wkplcTyp} </Text></View>
              <View style={styles.tableCell}><Text style={[styles.labelTextNarrowMore]}>:{WA1120Data.wkplc}</Text></View>
            </View>
            <View style={styles.tableRow}>
              <View style={styles.tableCell}><Text style={[styles.labelTextNarrowMore,styles.alignRight]}>輸送車両 </Text></View>
              <View style={styles.tableCell}><Text style={[styles.labelTextNarrowMore]}>:{WA1120Car.carNo}</Text></View>
            </View>
            <View style={styles.tableRow}>
              <View style={styles.tableCell}><Text style={[styles.labelTextNarrowMore,styles.alignRight]}>運転手 </Text></View>
              <View style={styles.tableCell}><Text style={[styles.labelTextNarrowMore]}>:{WA1120Drv.drvNm}</Text></View>
            </View>
            <View style={styles.tableRow}>
              <View style={styles.tableCell}><Text style={[styles.labelTextNarrowMore,styles.alignRight]}>行先名 </Text></View>
              <View style={styles.tableCell}><Text style={[styles.labelTextNarrowMore]}>:{WA1120Dest.fixPlacNm}</Text></View>
            </View>
            <View style={styles.tableRow}>
          ...
          <View  style={[styles.center]}>
            <Text style={styles.labelTextNarrowMore}>数:可燃:{WA1121Data.flamNm} 不燃:{WA1121Data.nonFlamNm} 飛灰:{WA1121Data.flyAshNm} 主灰:{WA1121Data.bottomAshNm}</Text>
          </View>
          <View style={styles.tableMain}>
            <View style={styles.tableRow}>
          ...
          {showScannerTag && (
              <Modal visible={showScannerTag} onRequestClose={() => setShowScannerTag(false)}>
                  <QRScanner onScan={handleCodeScannedForTag} closeModal={() => setShowScannerTag(false)} isActive={showScannerTag} errMsg={"タグ読込"}/>
              </Modal>
          )}
