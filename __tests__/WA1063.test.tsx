...
jest.mock('../src/utils/Api', () => ({
    IFA0320: jest.fn<Promise<ApiResponse<IFA0320Response<IFA0320ResponseDtl>>>, [string, string]>(),
}));
...
describe('WA1063 レンダリング', () => {
    ...
    it('レンダリング(color=1のとき) -> 111111111確認', async () => {
        ...
        const { getByText, findByText } = render(
            <RecoilRoot initializeState={(snapshot) => snapshot.set(mockState, mockedState)}>
                <WA1063 navigation={mockNavigation} />
            </RecoilRoot>
        )
